...
class ProjectionBall():
    ...
    def grad_conj(self, y):
        n_y = np.linalg.norm(y - self.center, axis=-1, ord=2)
        return self.center + (y - self.center) / np.maximum(1, n_y/self.radius)[:,np.newaxis]
    
    def hessian(self, theta):
        n,m = theta.shape
        return np.dot(np.ones(theta.shape), np.eye(m) )
        # to be verified !! -- same as L2 case
...
class LogBarrier():
    ...
    def hessian(self, theta):
        n,m = theta.shape
        return np.dot(((1/(1-theta))**2 + (1/(1+theta))**2), np.eye(m))
...
class ElasticNet():
    ...
    def hessian(self, theta):
        n,m = theta.shape
        I = 1/self.delta * np.dot(np.ones(theta.shape), np.eye(m))
